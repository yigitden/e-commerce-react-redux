...
const ProductCard = ({ product, index }) => {
  ...
  const productListAmount = productsArrayInCard.find(
    (item) => item.id == product.id
  );
  return (
    <div className="col-12-xs col-6-sm col-4-md col-3-xl" key={index}>
      <div className="card">
        <div className="card-image">
          <img src={product.image} />
        </div>
        <div className="row  justify-space-between category">
          <div className="card-category">{product.category.toUpperCase()}</div>
        ...
        <div className="product-card-button">
          {productListAmount && productListAmount.amount ? (
            ...
            <div
              className="row justify-center card-button"
              onClick={() => dispatch(addToCard(productInfo))}
            >
              ADD TO CARD
            </div>
          )}
